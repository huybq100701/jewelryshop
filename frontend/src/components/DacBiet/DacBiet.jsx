...
const DacBiet = () => {
  ...
  useEffect(() => {
    const laySachDacBiet = async () => {
      const res = await axios.get(`${apiUrl}api/jewels?sapxep=moi`);
  ...
  return (
    <div className="DacBiet">
      <div className="DacBiet__title">
        ...
        <p>
          Một trong những yếu tố làm nên sự đặc biệt của các phiên bản này là
          chúng thường được làm với số lượng ít. Bởi vậy, chúng còn được gọi là
        ...
        {dataDacBiet
          ? dataDacBiet.map((item, index) => {
              return (
                <div
                  className={
                    Math.abs(index - dacBietSlide) < 3 && index >= dacBietSlide
                      ? "DacBiet__item active"
                      : "DacBiet__item"
                  }
                  key={index}
                >
      ...
      <div className="DacBiet__nut">
        <ArrowBackOutlined
          className="DacBiet__nut__icon"
          onClick={nutLuiDacBetSlide}
        />
        <ArrowForwardOutlined
          className="DacBiet__nut__icon"
          onClick={nutTienDacBietSlide}
        />
      </div>
    </div>
  );
