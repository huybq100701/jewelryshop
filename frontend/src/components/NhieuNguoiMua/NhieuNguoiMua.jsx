...
const NhieuNguoiMua = () => {
  ...
  useEffect(() => {
    const laySachNhieuNguoiMua = async () => {
      const res = await axios.get(`${apiUrl}api/jewels?sapxep=luotmua`);
    ...
    <div className="NhieuNguoiMua">
      <div className="NhieuNguoiMua__title">
        ...
        <div className="NhieuNguoiMua__title__nut">
          <ArrowBackIosNewOutlined
            className="NhieuNguoiMua__title__nut__icon"
            onClick={nutLuiNhieuNguoiMuaSlide}
          />
          {[0, 1, 2, 3].map((item) => {
            return (
              <Circle
                key={item}
                className={item === nhieuNguoiMuaSlide ? "dot active" : "dot"}
              />
            );
          })}
          <ArrowForwardIosOutlined
            className="NhieuNguoiMua__title__nut__icon"
            onClick={nutTienNhieuNguoiMuaSlide}
          />
        </div>
      </div>
          ...
          ? dataNhieuNguoiMua.map((item, index) => {
              if (index < 9)
                return (
                  <div
                    className={
                      Math.abs(index - nhieuNguoiMuaSlide) < 6 &&
                      index >= nhieuNguoiMuaSlide
                        ? "NhieuNguoiMua__sach__item active"
                        : "NhieuNguoiMua__sach__item"
                    }
                    key={index}
                  >
                    <Link
                    ...
                    >
                      <div className="NhieuNguoiMua__sach__item__hinhAnh">
                        <img
                          src={`${apiUrl}images/jewels/${item.imgUrl}`}
                          alt=""
                        />
                      </div>
